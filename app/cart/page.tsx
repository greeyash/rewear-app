  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      for (const item of selectedItemsData) {
        if (item.products.status === 'sold') {
          alert(`${item.products.product_name} sudah terjual. Mohon hapus dari keranjang.`);
          setCheckingOut(false);
          return;
        }

        if (item.quantity > item.products.quantity) {
          alert(`Stok ${item.products.product_name} tidak mencukupi. Maksimal ${item.products.quantity} item.`);
          setCheckingOut(false);
          return;
        }
      }
      ...
      const checkoutData = {
        items: selectedItemsData.map(item => ({
          ...
          photo: item.products.photos?.front,
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-32">
      ...
      {items.length === 0 ? (
        ...
        <>
          {/* Select All */}
          <div className="bg-white px-4 py-3 flex items-center justify-between mt-2">
            <button
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.quantity > item.products.quantity && (
                      <div className="text-xs text-red-600 mb-1 flex items-center gap-1">
                        ...
                        Stok hanya {item.products.quantity}
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="w-7 h-7 rounded-full border-2 border-gray-300 flex items-center justify-center disabled:opacity-30 hover:border-[#66bb6a] transition-colors"
                        >
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-600">
                Total ({getSelectedItemsCount()} item)
              </p>
